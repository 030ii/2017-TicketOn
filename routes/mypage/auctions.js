        ...
        async.series([
            ...
            function(callback) {
                ...
                query = "SELECT A.aid, B.b_price, U.u_name FROM auction AS A JOIN bid AS B JOIN user AS U "
                        + "WHERE B.b_price IN (SELECT MAX(b_price) FROM bid GROUP BY aid) AND B.aid = A.aid AND B.uid = U.uid AND A.uid=?"
                connection.query(query, req.session.uid, function(err, rows) {
                    if(err) callback(err);
                    callback(null, rows);
                });
            ...
            function(callback) {
                // 상태별 경매 수 정보 조회
                query = "SELECT a_status, count(*) AS cnt FROM auction WHERE uid=? GROUP BY a_status";
                connection.query(query, req.session.uid, function(err, rows) {
                    if(err) callback(err);
                    var status = [0,0,0,0];
            ...
            function(callback) {
                // 입찰 횟수 조회
                query = "SELECT aid, count(*) AS cnt FROM bid WHERE aid IN"
                    + " (SELECT aid FROM auction WHERE uid=?) GROUP BY aid";
                connection.query(query, req.session.uid, function(err, rows) {
                    if(err) callback(err);
                    callback(null, rows);
                });
